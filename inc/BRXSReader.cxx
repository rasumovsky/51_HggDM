...
//  cross sections are in units of pb. Tools for accessing the DM and other   //
//  MC cross-sections are also included.                                      //
//                                                                            //
...
/**
   ...
   @param value - The value (XS, +ERR, -ERR, FEFF).
...
/**
   -----------------------------------------------------------------------------
   Get the interpolated SM XS or BR value.
   @param mass - The mass point for which we are getting values.
   @param mapType - XS or BR.
   @param process - The signal process.
...
/**
   -----------------------------------------------------------------------------
   Convert variable information to a key for map storage.
   @param sampleName - The name of the MC sample.
   @param value - The value (XS, FEFF, NEVT).
   @returns - The map key.
*/
TString BRXSReader::getMCMapKey(TString sampleName, TString value) {
  TString key = Form("%s_%s", sampleName.Data(), value.Data());
  return key;
}

/**
   -----------------------------------------------------------------------------
   Returns the cross-section, filter efficiency, or number of events for a MC.
   @param sampleName - The MC sample.
   @param value - The value (FEFF, XS, NEVT).
*/
float BRXSReader::getMCXS(TString sampleName, TString value) {
  TString currKey = getMCMapKey(sampleName, value);
  if (hasKey(currKey, "XS")) {
    return valuesXS[currKey];
  }
  else {
    std::cout << "BRXSReader: Error! Undefined MC sample request: "
	      << sampleName << ", " << value << std::endl;
    exit(0);
  }
}
...
/**
   -----------------------------------------------------------------------------
   Get the two defined mass points that are closest to the test mass.
   @param testMass - The point for which we are finding neighbors.
   @param mapType - BR or XS map to use.
   @returns - A pair of mass points (above and below the testMass value).
*/
...
void BRXSReader::loadSMBR(TString decayClass) {
  ...
  TString fileName = Form("%s/%s", directory.Data(),(m_config->getStr(Form("FileBR%s",decayClass.Data()))).Data());
...
void BRXSReader::loadDMXS() {
  ...
  TString fileName = Form("%s/%s", directory.Data(), (m_config->getStr("FileXSDM")).Data());
  ifstream currFile(fileName);
  if (!currFile) {
    std::cout << "BRXSReader! Error loading file: " << std::endl;
    std::cout << "\t" << fileName << std::endl;
    ...
    while (!currFile.eof()) {
      currFile >> mediatorName >> mediatorMass >> fermionMass
	       >> currIn[0] >> currIn[1] >> currIn[2] >> currIn[3];
...
/**
   -----------------------------------------------------------------------------
   Load the cross-section data for all other Monte Carlo samples.
*/
void BRXSReader::loadMCXS() {
  std::cout << "  BRXSReader: loadMCXS()" << std::endl;
  TString fileName = Form("%s/%s", directory.Data(), (m_config->getStr("FileXSMC")).Data());
  ifstream currFile(fileName);
  if (!currFile) {
    std::cout << "BRXSReader! Error loading file: " << std::endl;
    std::cout << "\t" << fileName << std::endl;
    ...
    while (!currFile.eof()) {
      currFile >> sampleName >> generatedEvts >> xsection >> filterEff;
      valuesXS[getMCMapKey(sampleName, "NEVT")] = (float)generatedEvts;
      valuesXS[getMCMapKey(sampleName, "XS")] = xsection;
      valuesXS[getMCMapKey(sampleName, "FEFF")] = filterEff;
    }
  }
  currFile.close();
}
...
void BRXSReader::loadSMXS(TString production) {
  ...
  TString fileName = Form("%s/%s", directory.Data(), (m_config->getStr(Form("FileXS%s",production.Data()))).Data());
  ifstream currFile(fileName);
  if (!currFile) {
    std::cout << "BRXSReader! Error loading file: " << std::endl;
    std::cout << "\t" << fileName << std::endl;
