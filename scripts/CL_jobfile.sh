...
if [[ $# -lt 5 ]]; then
    echo "USAGE: CL_jobfile.sh <jobname> <input_file> <exe_name> <lambda> <lifetime>"
    ...
    echo $jobname $input_file $exe_name $lambda $lifetime
        
    out="${jobname}_${lambda}_${lifetime}"
    output_dir="/afs/cern.ch/user/a/ahard/work_directory/files_NPP/FullAnalysis/${jobname}/CL_limits"
...
# Go!
    ...
    ./bin/${exe_name} ${jobname} ${lambda} ${lifetime} 1> ${out}.log 2>${out}.err;
