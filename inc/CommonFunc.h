...
namespace CommonFunc{
  TH1F* CreateHist(TString title="hist",TString tag="",TString axisx="",
		   TString axisy="",int nbin=100,double xmin=0,double xmax=100,
		   double min=0);
  TH1D* CreateTH1D(TString title="hist",TString tag="",TString axisx="",
		   TString axisy="",int nbin=100,double xmin=0,double xmax=100,
		   double min=0);
  TCanvas* CreateCanvas(TString title="canvas",TString tag="",double ww=800,
			double wh=600);
  TPad* CreatePad(TString title="pad",TString tag="",double x1=0,double y1=0.,
  ...
  TLegend* CreateLegend(double x_low, double y_low, double x_up, double y_up, 
			TList* h_list, std::vector<TString> text,
			std::vector<TString> option,double textsize);
  TLegend* FastLegend(double x_low, double y_low, double x_up, double y_up,
		      double textsize);
  void DrawConstantLine(TVirtualPad *c, double value,double *mass,int nmass,
			Color_t color=2, int linestyle=2, double linewidth=3);
  void DrawConstantLine(TVirtualPad *c, double value, double x1, double x2,
			Color_t color=2, int linestyle=2, double linewidth=3);
  void DrawVerticalLine(TVirtualPad *c, double value, double y1, double y2,
			Color_t color=2, int linestyle=2, double linewidth=3);
  TPaveText* CreatePaveText(double x1,double y1,double x2,double y2,
			    std::vector<TString> text,double textsize);
  TStyle* AtlasStyle();
  void SetAtlasStyle();
  void ScaleToOne(TH1*);
